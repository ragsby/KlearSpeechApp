...
import { consonants, cvSentences } from '../../constants';
...
const Level4Content: React.FC<Level4ContentProps> = ({ isComplete, onToggleComplete }) => {
  ...
  const generateRandomStory = () => {
    ...
    const newStory = randomSelection.map(c => cvSentences[c] || `${c} का वाक्य उपलब्ध नहीं है`).join('. ') + '.';
    setStory(newStory);
  };

  const handleSelect = (consonant: string) => {
    setSelected(consonant === selected ? '' : consonant);
  };

  return (
    <div className="space-y-3">
      <div>
        <label className="block text-lg font-medium text-gray-600 mb-2">Select Hindi Consonant:</label>
      ...
      <div className={`p-3 rounded-lg text-lg transition-all duration-300 flex items-start gap-2 min-h-[4rem]
        ...
        <span>{selected ? cvSentences[selected] : 'Choose a consonant to see the practice sentence.'}</span>
      </div>
      
      <button onClick={generateRandomStory} className="w-full py-3 bg-accent text-white rounded-full text-lg font-semibold hover:bg-accent-dark transition-all duration-300 flex items-center justify-center gap-2 transform hover:scale-105 shadow-md">
        <Shuffle size={18} />
        Random CV Story (15 sentences)
      </button>
      
      {story && 
        <div className="mt-2 p-4 bg-white border border-gray-200/80 rounded-lg text-gray-700 animate-fade-in">
          <h4 className="font-semibold text-xl text-accent-dark mb-2 flex items-center gap-2"><BookOpen size={20}/> Story Time</h4>
          <p className="text-lg leading-relaxed">{story}</p>
        </div>
      }

      <div className="mt-2">
        <label htmlFor="level4-complete" className="flex items-center gap-3 cursor-pointer p-3 rounded-lg hover:bg-gray-100/50">
            <div className="relative flex items-center h-5">
                <input type="checkbox" checked={isComplete} onChange={onToggleComplete} id="level4-complete" className="sr-only peer"/>
